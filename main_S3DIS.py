...
# from RandLA_BE import Network
from RandLA_BE_DR import Network
# from RandLA_BE_DR_MF import Network
...
class S3DIS:
    def __init__(self, test_area_idx):
        self.name = 'S3DIS'
        self.path = '/root/autodl-tmp/Stanford3dDataset_v1.2_Aligned_Version'
        ...
        self.all_files = glob.glob(join(self.path, 'input_0.040', '*.ply'))
    ...
    def get_tf_mapping2():
        ...
        def tf_map(batch_xyz, batch_features, batch_labels, batch_pc_idx, batch_cloud_idx):
            ...
            for i in range(cfg.num_layers):
                ...
                sub_rgb = batch_rgb[:, :tf.shape(batch_xyz)[1] // cfg.sub_sampling_ratio[i], :]
                pool_i = neighbour_idx[:, :tf.shape(batch_xyz)[1] // cfg.sub_sampling_ratio[i], :]
                up_i = tf.py_func(DP.knn_search, [sub_points, batch_xyz, 1], tf.int32)
                input_points.append(batch_xyz)
                input_rgb.append(batch_rgb)
            ...
            input_list = input_points + input_rgb + input_neighbors + input_pools + input_up_samples
